...
在完成了类SRAM接口的CPU设计基础上，额外添加一个类SRAM-AXI转接桥，用于将CPU的两个类SRAM接口连接到AXI接口。除了完成通信协议的转换外，转接桥还需要对传入的类SRAM请求进行仲裁。为了简化设计，读请求和写请求不能并行进行，即便它们使用不同的通道。

\subsection{处理器结构设计框图}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fig/CPU_arch_prj5.png}
    \caption{处理器结构设计框图}
    \label{fig:CPU_arch_prj5}
\end{figure}
...
\subsubsection{类SRAM-AXI转接桥设计}

在类SRAM-AXI转接桥中，需要实现以下两个功能：

\begin{itemize}
    \item 对CPU发出的取指请求和访存请求进行仲裁，确保至多只同时处理一个请求；

    \item 将CPU使用的类SRAM总线连接至AXI总线。
\end{itemize}

在本次设计中，使用了一个单独的状态机来实现整个转接桥，状态转移图如下。以下分别对于每个状态的工作进行分析：

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{fig/bridge_state.png}
    \caption{转接桥状态转移图}
    \label{fig:bridge_state}
\end{figure}

\begin{itemize}
    \item \textbf{S_IDLE}
      S_IDLE状态等待处理器发起请求。其中，\texttt{grant}信号标记当前正在处理的请求（0为取指，1为访存，下同），\texttt{last_grant}信号标记上次处理的请求。如果两个请求同时有效，则使用Round Robin的方式防止出现饥饿。同时，在S_IDLE状态需要根据请求为读请求或写请求，分别跳转至S_AR状态或S_AW状态。
...
\end{lstlisting}
    ...
    \item S_AR状态负责处理读地址的发送。其中需要注意的是，类SRAM接口支持在握手发生之前取消当前请求，若请求取消，则回到S_IDLE状态。AR通道的握手状态通过信号\texttt{ar_hs = (state == `S_AR) \&\& sram_req[grant] \&\& arready}追踪，若握手成功则进入S_R状态。

    \item S_R状态负责处理读数据的接收，只需通过\texttt{r_hs = (state == `S_R) \&\& rvalid}追踪握手状态，在握手成功后回到S_IDLE状态即可。

    \item S_AW状态负责处理写地址和写数据的发送。除了要考虑握手前的请求取消以外，由于要处理两个通道的握手，此处引入了寄存器\texttt{wready_buf}用于分别追踪两个通道的握手情况。
...
\end{lstlisting}
      ...
      其中，\texttt{aw_done}和\texttt{w_done}信号用于记录两个通道是否\textbf{已经}完成握手，\texttt{aw_hs}和\texttt{w_hs}用于记录两个通道\textbf{在当前这一拍}是否\textbf{正在}握手。只有当两个通道在当前这一拍都握手成功了，才可以进入S_B状态。
...
\end{lstlisting}
    ...
    \item S_B状态用来处理写请求的返回，和S_R类似，只需通过\texttt{b_hs = (state == `S_B) \&\& bvalid}追踪握手状态，在握手成功后回到S_IDLE状态即可。

\end{itemize}

在核心状态机设计完毕后，便可以根据当前状态设置相应的握手信号值。需要注意\texttt{awvalid}信号和\texttt{wvalid}信号只有在握手还未成功时拉高，避免出现重复握手的情况。
...
\subsection{br_taken信号导致的数据请求饥饿问题}

在最初的设计中，转接桥中没有仲裁机制，在数据请求的优先级高于指令请求的时候，可以通过测试。但在指令请求的优先级高的时候，仿真会阻塞在pc=0x1c032e40的位置。

简单分析发现，此时EX级与IF级同时向转接桥发送请求。由于指令请求的优先级高，故转接桥会选择指令请求处理。但是问题出在IF级一直在循环向转接桥发送同一个指令请求，导致转接桥循环处理同样的指令请求，使得数据请求饥饿。

理想情况下，当pre-IF级的指令请求接收到 addr_ok 信号后,指令就应该进入IF级并将IF级的valid信号置1。由于后续的流水线级已经陷入阻塞，故IF级也会陷入阻塞，使得if_allowin信号拉低，此时pre-IF级便不再继续发送请求。

但是\ref{fig:debug2}中的信号显示 if_allowin 信号只被拉高了一拍。原因是此时阻塞在ID级的恰好是一条 br_taken 的分支指令，导致 if_cancel 信号被拉高。取来的指令在存留一拍之后就被取消，导致IF认为自己仍空，促使pre-IF级反复发送同一请求，陷入循环。

在本次实验中，我们采取了简单的修复措施：在转接桥中加入一个简单的 Round Robin 仲裁机制，从而避免数据请求的饥饿。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fig/debug2.png}
    \caption{数据请求饥饿的调试波形}
    \label{fig:debug2}
\end{figure}
...
经过对CPU设计的修改和调试，我们成功地将原有的SRAM接口改造为类SRAM总线接口。将类SRAM接口的CPU与转接桥连接，实现了CPU通过AXI总线对内存的访问，并通过了固定延迟和随机延迟测试。

\section{小组成员分工}

王荦璠：负责exp14（类SRAM总线接口）的设计与实现，包括IFU、EXEU、MEMU等多个模块的修改，流水线控制逻辑的调整，以及相关问题的调试与修复。

陈琛：负责类SRAM-AXI转接桥的部分实现。

张钧玮：完善类SRAM-AXI转接桥，使之支持请求的更改与取消，并添加仲裁机制。
